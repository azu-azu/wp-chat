...
def enable_canary(percentage: float, base_url: str = "http://localhost:8080"):
    ...
    result = make_request(
        "POST", f"{base_url}/admin/canary/enable", {"percentage": percentage, "updated_by": "cli"}
    )
...
def disable_canary(base_url: str = "http://localhost:8080"):
    ...
    result = make_request("POST", f"{base_url}/admin/canary/disable", {"updated_by": "cli"})
...
def update_rollout(percentage: float, base_url: str = "http://localhost:8080"):
    ...
    result = make_request(
        "POST", f"{base_url}/admin/canary/rollout", {"percentage": percentage, "updated_by": "cli"}
    )
...
def emergency_stop(base_url: str = "http://localhost:8080"):
    ...
    result = make_request("POST", f"{base_url}/admin/canary/emergency-stop", {"updated_by": "cli"})
...
def clear_emergency(base_url: str = "http://localhost:8080"):
    ...
    result = make_request("POST", f"{base_url}/admin/canary/clear-emergency", {"updated_by": "cli"})
    ...
    for user_id in user_ids:
        try:
            response = requests.get(
                f"{base_url}/search", params={"q": "test query", "user_id": user_id, "topk": 1}
            )
            response.raise_for_status()
            data = response.json()
            ...
            print(
                f"   User {user_id}: Rerank {'✅' if rerank_enabled else '❌'} (Rollout: {rollout_pct:.1%})"
            )
...
def main():
    ...
    enable_parser.add_argument("percentage", type=float, help="Rollout percentage (0.0 to 1.0)")
    ...
    update_parser.add_argument("percentage", type=float, help="Rollout percentage (0.0 to 1.0)")
    ...
    test_parser.add_argument("user_ids", nargs="+", help="User IDs to test")
