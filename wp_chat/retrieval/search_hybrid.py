...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--wd", type=float, default=None, help="Dense weight (uses config.yml if not specified)"
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="MMR lambda (uses config.yml if not specified)",
    )
    ap.add_argument(
        "--scoring-strategy", default="default", help="Scoring strategy for composite scoring"
    )
    args = ap.parse_args()
    main(
        args.query, args.topk, args.wd, args.wb, args.rerank, args.mmr_lambda, args.scoring_strategy
    )
