...
def retrieve_hybrid(q: str, topk: int, wd=0.6, wb=0.4, rerank_mode="none", mmr_lambda=0.7) -> Tuple[List[Tuple[int, float]], dict]:
...
def evaluate(mode: str, k: int, eval_path: str, rerank_mode: str = "none", mmr_lambda: float = 0.7):
    ...
    def retrieve(q: str):
        ...
        if mode == "hybrid":
            results, rerank_info = retrieve_hybrid(q, k, rerank_mode=rerank_mode, mmr_lambda=mmr_lambda)
            return results
